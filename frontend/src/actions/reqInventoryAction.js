...
const userID=JSON.parse(info)._id;
  ...
  try {
    ...
    const { data } = await axios.get(`http://localhost:4000/api/v1/requests/me/${userID}`);
...
export const deleteRequest = (id) => async (dispatch) => {
  try {
    ...
    const { data } = await axios.delete(`http://localhost:4000/api/v1/admin/request/${id}`);
...
export const getRequestDetails = (id) => async (dispatch) => {
  try {
    ...
    const { data } = await axios.get(`http://localhost:4000/api/v1/reqInventory/${id}`);
