...
exports.createReqInventory = catchAsyncErrors(async (req, res, next) => {
  const { orderItems, department,user } = req.body;
...
exports.myRequests = catchAsyncErrors(async (req, res, next) => {
  const request = await ReqInventory.find({ user: req.params.id });
...
exports.getAllReq = catchAsyncErrors(async (req, res, next) => {
  const users = await ReqInventory.find();
